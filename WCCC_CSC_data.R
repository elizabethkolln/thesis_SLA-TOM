...
#Last edited by: Elizabeth Kolln 01/14/2025
...
setwd("~/Desktop/wellesley/thesis/final_data/WCCC_and_CSC")

###importing the TOM data###
TOM_data <- read_csv("CSC_WCCC_TOM.csv")
...
SLA_data <- read.csv("CSC_WCCC_SLA.csv")
...
EF_PVT_data <- read.csv("CSC_WCCC_EF_PVT.csv")
...
#adding ages to IDs
ID_ages <- read.csv("CSC_WCCC_ages.csv")
...
df_graph1$IDS <- (as.integer(SLA_cleaned$IDS1)
                  + as.integer(SLA_cleaned$IDS2)
                  + as.integer(SLA_cleaned$IDS3))

df_graph1$ADS <- (as.integer(SLA_cleaned$ADS1)
                  + as.integer(SLA_cleaned$ADS2)
                  + as.integer(SLA_cleaned$ADS3))

df_graph1$PDS <- (as.integer(SLA_cleaned$PDS1)
                  + as.integer(SLA_cleaned$PDS2)
                  + as.integer(SLA_cleaned$PDS3))

df_graph1$FDS <- (as.integer(SLA_cleaned$FDS1)
                  + as.integer(SLA_cleaned$FDS2)
                  + as.integer(SLA_cleaned$FDS3))
...
df_graph_SLA2 <- pivot_longer(df_graph_SLA1, c(IDS, ADS, PDS, FDS), 
               names_to = "Type",
               values_to = "Score")
...
ggplot(df_graph_SLA2, aes(x=Age_Mo, y=Score, fill=Type)) + 
  geom_area()
